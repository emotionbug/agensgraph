...
static void substituteGraphSetPropsAsTargetEntry(ParseState *pstate,
												 Query *query,
												 List *graphSetProps);
...
transformCypherSetClause(ParseState *pstate, CypherClause *clause)
{
	...
	substituteGraphSetPropsAsTargetEntry(pstate, qry, qry->graph.sets);
...
/*
 * substituteGraphSetPropsAsTargetEntry
 */
static void
substituteGraphSetPropsAsTargetEntry(ParseState *pstate, Query *query,
									 List *graphSetProps)
{
	ListCell   *cell;
...
find_target_label_walker(Node *node, find_target_label_context *ctx)
{
	...
	if (IsA(node, Var))
	{
		 ...
		 */
		if ((qry->graph.writeOp == GWROP_DELETE) &&
